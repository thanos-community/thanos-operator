...
Configure the cert-manager controller with `--enable-certificate-owner-ref=true` so issued Secrets are garbage-collected when their Certificates are deleted. This setting defaults to false and applies to all Certificates handled by that cert-manager installation. For a Helm installation, add it to the cert-manager chart values:
...
Each Thanos component custom resource owns its leaf Certificates; cert-manager sets ownership of their Secrets. Without this setting, deleting a Certificate retains its Secret.
...
The shared namespace CA is retained independently of individual Thanos custom resources. Deleting one component must not remove trust used by the remaining components. Namespace trust reconciliation continues after the last component resource is deleted, for as long as this operator runs with TLS enabled. Leaf Certificates and HTTP ConfigMaps are owned by the Thanos custom resource. Component controllers use the existing resource pruner to remove them when TLS is disabled or a shard is removed. With `--enable-certificate-owner-ref=true`, cert-manager makes each issued Secret a child of its Certificate, so Kubernetes garbage collection removes the private key with the Certificate. The shared CA Certificate is retained, preserving its Secret. Generated Certificates and ConfigMaps follow the same create-or-update behavior as other generated resources.
...
Check Certificate readiness in the workload namespace. In automatic mode, `thanos-operator-ca` must be issued before the TLS controller can publish trust. Workload and leaf Certificate resources can be created independently. Kubernetes waits for the required Secret and ConfigMaps before starting the pod. A missing cert-manager installation prevents a TLS-enabled operator from setting up its certificate watches.

For an existing issuer, first check its readiness and the referenced ConfigMap's namespace and data key. An empty or invalid CA bundle produces a TLS controller reconciliation error; component resource creation does not wait for that validation. A well-formed but unrelated CA bundle passes configuration checks and then causes client certificate verification failures. The operator does not silently switch generated clients to plaintext or disable verification on these failures.
...
Each component builder sets its own TLS flags in its argument builder and constructs its TLS mounts, probes, and pod-template checksum. It returns the workload's leaf Certificate and HTTP ConfigMap through `manifests.AppendTLSResources`. Cert-manager issues the leaf Secret referenced by the workload.
...
When TLS is enabled, the controller functions that assemble builder options use the handler's generic `GetConfigMapChecksum` method to fetch the public trust checksum alongside inputs such as discovered endpoints. They return the completed options to `syncResources` for building and applying resources. Sharded builders share one checksum read across their shards. Receive validates its replication protocol through its builder options. The generic resource handler applies the returned objects with the Thanos custom resource as owner. There is no TLS preparation step between building and applying resources. Trust validation belongs to the TLS controller; missing trust or leaf Secrets do not prevent the component resources from being created.

TLS-enabled workload controllers watch their owned leaf Certificates for repair and the configured public trust ConfigMap for checksum changes. HTTP ConfigMaps use the existing ownership watches. Shared CA Certificate, CA Secret, and Issuer events are handled by the TLS controller. Publishing a changed trust bundle requeues Thanos resources in the same namespace. Only the public trust bundle is hashed onto the pod template. The TLS controller handles only shared trust events; cert-manager manages issued Secret ownership. These cert-manager watches are absent when the feature is disabled.
...
The Go dependency uses cert-manager v1.19.4 API types to limit unrelated dependency upgrades. This is independent of the cert-manager controller version installed in the cluster; the existing E2E setup installs cert-manager v1.21.0 with `--enable-certificate-owner-ref=true`.
