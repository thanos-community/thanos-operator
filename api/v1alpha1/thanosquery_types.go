...
type SelectorRelabelConfig struct {
	// Action is the relabel action: keep, drop, hashmod, etc.
...
// Root and middle queriers set QueryAsStoreAPILabelSelector to discover other query
// services as StoreAPIs. Leaf queriers are normal ThanosQueries with no QueryFederation.
// +kubebuilder:validation:Optional
type QueryFederationConfig struct {
	// QueryAsStoreAPILabelSelector enables discovering other query services as StoreAPIs.
	// Set this on root and middle queriers to discover and query downstream queriers.
	// +kubebuilder:validation:Optional
	QueryAsStoreAPILabelSelector *metav1.LabelSelector `json:"QueryAsStoreAPILabelSelector,omitempty"`
	...
	// TSDBSelector adds relabel rules for the TSDB selector (--selector.relabel-config).
	...
	// +kubebuilder:validation:Optional
	TSDBSelector *[]SelectorRelabelConfig `json:"tsdbSelector,omitempty"`
}
