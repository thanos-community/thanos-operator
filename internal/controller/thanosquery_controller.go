...
func (r *ThanosQueryReconciler) buildQuery(ctx context.Context, query monitoringthanosiov1alpha1.ThanosQuery) (manifests.Buildable, error) {
	...
	if query.Spec.QueryFederation != nil && query.Spec.QueryFederation.DownstreamQueryStoreAPISelector != nil {
...
func (r *ThanosQueryReconciler) getQueryStoreAPIServiceEndpoints(ctx context.Context, query monitoringthanosiov1alpha1.ThanosQuery) ([]manifestquery.Endpoint, error) {
	labelSelector, err := manifests.BuildLabelSelectorFrom(query.Spec.QueryFederation.DownstreamQueryStoreAPISelector, requiredQueryServiceLabels)
	if err != nil {
		return []manifestquery.Endpoint{}, err
	}
...
func (r *ThanosQueryReconciler) enqueueForService() handler.EventHandler {
	return handler.EnqueueRequestsFromMapFunc(func(ctx context.Context, obj client.Object) []reconcile.Request {
		...
		for _, query := range queriers.Items {
			...
			if isQueryService && query.Spec.QueryFederation != nil && query.Spec.QueryFederation.DownstreamQueryStoreAPISelector != nil {
				selector, err := manifests.BuildLabelSelectorFrom(query.Spec.QueryFederation.DownstreamQueryStoreAPISelector, requiredQueryServiceLabels)
				if err == nil && selector.Matches(labels.Set(obj.GetLabels())) {
					shouldEnqueue = true
				}
			}
