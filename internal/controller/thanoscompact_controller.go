...
func (r *ThanosCompactReconciler) syncResources(ctx context.Context, compact monitoringthanosiov1alpha1.ThanosCompact) error {
	...
	for _, resource := range expectResources {
		builder := manifestcompact.Options{Options: manifests.Options{Owner: resource, Namespace: compact.GetNamespace()}}
		if objs := getFeatureGatedObjectsToDelete(builder, compact.Spec.FeatureGates); len(objs) > 0 {
