...
func TestExternalIssuerAndWorkloadLifecycle(t *testing.T) {
	...
	require.NoError(t, m.Client.Create(ctx, secret))
	require.NoError(t, m.EnsureWorkload(ctx, workload))
	require.NoError(t, m.SetChecksum(ctx, workload))
	first := workload.Spec.Template.Annotations[manifests.TLSChecksumAnnotation]
	require.NoError(t, m.SetChecksum(ctx, workload))
	require.Equal(t, first, workload.Spec.Template.Annotations[manifests.TLSChecksumAnnotation])
	...
	require.NoError(t, m.Client.Update(ctx, secret))
	require.NoError(t, m.SetChecksum(ctx, workload))
	require.NotEqual(t, first, workload.Spec.Template.Annotations[manifests.TLSChecksumAnnotation])
	second := workload.Spec.Template.Annotations[manifests.TLSChecksumAnnotation]
	bundle.Data["roots.pem"] += string(rootPEM(t))
	require.NoError(t, m.Client.Update(ctx, bundle))
	require.NoError(t, m.SetChecksum(ctx, workload))
	require.NotEqual(t, second, workload.Spec.Template.Annotations[manifests.TLSChecksumAnnotation])
